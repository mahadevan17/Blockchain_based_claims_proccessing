...
document.getElementById('createpresciption').addEventListener('click', () => {
  ...
  document.getElementById('ML_approval').addEventListener('click', async () => {
    ...
    let attending_doc=prescriptionData.AttendingPhysician; //4 attendingphysician
    let other_doc=prescriptionData.OtherPhysician;   //5 otherphysician
    let drug1=prescriptionData.drug1; //6 claimdignoticscode1
    let drug2=prescriptionData.drug2; //7 claimdignoticscode2
    let state=prescriptionData.State; //8 state
    let country=prescriptionData.Country; // 9 country
    let InscClaimAmtReimbursed=document.getElementById("InscClaimAmtReimbursed").value; // 10 InscClaimAmtReimbursed 
    ...
    let inpFeatures=[patientID,claimid,provider,attending_doc,other_doc,drug1,drug2,
      state,country,InscClaimAmtReimbursed,annReimbursed,annDeductible,age]; //input features for model
